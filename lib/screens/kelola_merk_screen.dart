...
class KelolaMerkScreen extends StatefulWidget {
  const KelolaMerkScreen({super.key});

  @override
  State<KelolaMerkScreen> createState() => _KelolaMerkScreenState();
}

class _KelolaMerkScreenState extends State<KelolaMerkScreen> {
  ...
  Future<void> _tambahMerk() async {
    final merk = _merkController.text.trim();
    if (merk.isEmpty) return;
    await _merkRef.add({'nama': merk});
    _merkController.clear();
  }

  Future<void> _hapusMerk(String id) async {
    await _merkRef.doc(id).delete();
  }
        ...
        child: Container(
          ...
          child: SafeArea(
            ...
            child: Row(
              children: const [
                SizedBox(width: 8),
                Expanded(child: Text('Kelola Merk', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold))),
              ],
            ),
          ),
        ),
